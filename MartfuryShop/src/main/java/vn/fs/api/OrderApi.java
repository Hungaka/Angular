...
//		cartDetailRepository.deleteByCart(cart);
	...
	@GetMapping("deliver1/{orderId}")
	public ResponseEntity<Void> deliver1(@PathVariable("orderId") Long id) {
		if (!orderRepository.existsById(id)) {
			return ResponseEntity.notFound().build();
		}
		Order order = orderRepository.findById(id).get();
	...
	@GetMapping("deliver2/{orderId}")
	public ResponseEntity<Void> deliver2(@PathVariable("orderId") Long id) {
		if (!orderRepository.existsById(id)) {
			return ResponseEntity.notFound().build();
		}
		Order order = orderRepository.findById(id).get();
	...
	@GetMapping("cancel1/{orderId}")
	public ResponseEntity<Void> cancel1(@PathVariable("orderId") Long id) {
		if (!orderRepository.existsById(id)) {
			return ResponseEntity.notFound().build();
		}
		Order order = orderRepository.findById(id).get();
